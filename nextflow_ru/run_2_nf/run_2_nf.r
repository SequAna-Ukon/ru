...
ggsave("nextflow_ru/run_2_nf/shaking_stacked_bars.png")
...
ggplot(pcaData_shaking, aes(PC1, PC2, color=time_hr, shape=cell_line)) +
  geom_point(size=3) +
  xlab(paste0("PC1: ",percentVar_shaking[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar_shaking[2],"% variance")) + 
  coord_fixed()

ggsave("nextflow_ru/run_2_nf/shaking_pca.png")
...
# Instead of using the TRUE or FALSE of the ...XXX
...
pcaData_shaking = plotPCA(vsd_non_shaking, intgroup=c("time_hr", "cell_line"), returnData=TRUE)
